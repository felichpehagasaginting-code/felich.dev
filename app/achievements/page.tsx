  ...
  return (
    <div className="relative inline-flex items-center justify-center" style={{ width: size, height: size }}>
      <svg width={size} height={size} className="-rotate-90">
        <circle cx={size / 2} cy={size / 2} r={radius} fill="none" stroke="currentColor" strokeWidth={strokeWidth} className="text-[var(--border-default)]" />
        <motion.circle
          cx={size / 2}
          cy={size / 2}
          r={radius}
      ...
      <div className="absolute flex flex-col items-center">
        ...
        <span className="text-[10px] text-[var(--text-muted)] font-medium">IPK</span>
      </div>
    </div>
  );
}
    ...
    <PageTransition>
      <div>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="mb-8">
          ...
          <p className="text-[var(--text-muted)] text-sm">A curated collection of certificates and awards I&apos;ve earned throughout my academic and professional journey.</p>
        </motion.div>

        <hr className="dotted-divider mb-8" />

        <div className="flex flex-wrap gap-2 mb-4">
          <span className="text-xs text-[var(--text-muted)] self-center mr-1">Total: {achievements.length}</span>
          ...
          <AnimatePresence mode="popLayout">
            {filtered.map((item: any, i) => (
              <motion.div 
                ...
                className={`group p-5 rounded-3xl border border-[var(--border-default)] bg-[var(--glass-bg)] backdrop-blur-xl hover:bg-[var(--bg-surface)] hover:shadow-[0_10px_40px_var(--brand-bg)] hover:-translate-y-1 hover:border-[var(--brand)]/30 transition-all duration-300 relative overflow-hidden ${item.link ? 'cursor-pointer' : ''}`}
              >
                <div className="absolute inset-0 bg-gradient-to-br from-[var(--bg-surface)] to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
                <div className="flex items-start gap-5 relative z-10">
                  <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${item.color} flex items-center justify-center text-white flex-shrink-0 shadow-lg group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300 ring-2 ring-[var(--bg-base)] inset-shadow-sm`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-3 flex-wrap">
                      <span className="px-2.5 py-1 text-[10px] font-bold rounded-md bg-[var(--bg-muted)] text-[var(--text-muted)] uppercase tracking-widest">{item.type}</span>
                      {item.year && <span className="text-[10px] font-mono font-semibold text-[var(--text-muted)] bg-[var(--bg-muted)] px-2 py-0.5 rounded">{item.year}</span>}
                      {item.link && (
                        <span className="text-[10px] font-bold text-[var(--brand)] bg-[var(--brand-bg)] px-2.5 py-1 rounded-md uppercase tracking-widest ml-auto flex items-center gap-1 group-hover:translate-x-1 transition-transform">
...
<p className="text-xs text-[var(--text-muted)] font-semibold mt-1">
            ...
            <div className="flex flex-wrap gap-1.5 justify-start sm:justify-end">
              {['AI & Machine Learning', 'Firebase', 'Go', 'JavaScript', 'Python'].map((interest) => (
                <span 
                  key={interest} 
                  className="px-2.5 py-1 text-[10px] font-bold rounded-full bg-[var(--bg-muted)] text-[var(--text-muted)] border border-[var(--border-default)] select-none cursor-default"
                >
                  {interest}
                </span>
              ))}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
            {googleBadges.map((badge, idx) => (
              <motion.div 
                ...
                className="p-4 rounded-2xl transition-all duration-300 relative group flex flex-col items-center justify-center text-center h-[140px] select-none hover:bg-[var(--bg-muted)] border border-transparent hover:border-[var(--border-default)] shadow-none hover:shadow-sm"
              >
                {/* Soft ambient glow */}
                ...
                <div className="mt-2.5 relative z-10 w-full">
                  <h4 className="font-bold text-[10px] leading-tight text-[var(--text-primary)] tracking-tight line-clamp-2 min-h-[1.5rem]">
                    {badge.title}
                  </h4>
                  <p className="text-[8px] font-mono text-[var(--text-muted)] font-semibold mt-1">
                    {badge.date}
                  </p>
                </div>
              </motion.div>
        ...
        <motion.section initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.5 }} className="mb-12">
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-[var(--text-muted)] text-sm font-medium">Teknologi Rekayasa Perangkat Lunak — Academic Index</p>
            </div>
            <div className="hidden sm:flex flex-col items-end">
               <span className="text-[10px] font-mono text-[var(--text-muted)] uppercase tracking-widest">Auth Level</span>
               <span className="text-xs font-black text-[var(--success)] uppercase">Administrator</span>
            </div>
          </div>
          ...
          <motion.div 
            ...
            whileInView={{ opacity: 1, y: 0 }} 
            viewport={{ once: true }} 
            className="p-1 rounded-[2rem] bg-gradient-to-br from-[var(--bg-muted)] via-[var(--bg-surface)] to-transparent mb-8"
          >
            <div className="p-6 md:p-8 rounded-[2rem] bg-[var(--glass-bg)] backdrop-blur-2xl border border-[var(--border-default)] overflow-hidden shadow-xl liquid-glass">
              <div className="flex flex-col md:flex-row items-center md:items-start justify-between gap-8 relative z-10">
                ...
                <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 text-center sm:text-left">
                  <div className="relative">
                    <div className="w-24 h-24 rounded-full bg-[var(--bg-muted)] p-0.5 ring-2 ring-[var(--border-default)] shadow-lg overflow-hidden">
                       <Image
                    ...
                    <div className="absolute bottom-0 right-0 w-6 h-6 bg-[var(--success)] rounded-full border-2 border-[var(--bg-base)] flex items-center justify-center shadow">
                  ...
                  <div className="space-y-2.5">
                    <div>
                      <h3 className="text-xl md:text-2xl font-black tracking-tight text-[var(--text-primary)] uppercase">Felich Pehagasa Ginting</h3>
                      <p className="text-sm font-semibold text-[var(--text-muted)]">D4 Teknologi Rekayasa Perangkat Lunak</p>
                    </div>
                    
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-[var(--text-muted)] font-medium justify-center sm:justify-start">
                ...
                <div className="flex flex-col items-center md:items-end justify-center gap-2.5 bg-[var(--bg-muted)] border border-[var(--border-default)] rounded-2xl p-4 min-w-[160px] text-center md:text-right shadow-inner">
                  ...
                  <div className="px-3 py-1 rounded-full bg-[var(--success)]/10 text-[var(--success)] text-[10px] font-black uppercase tracking-wider flex items-center gap-1.5 border border-[var(--success)]/30">
          ...
          <div className="mb-10">
            <div className="flex items-center gap-3 mb-6">
               <h3 className="text-lg font-extrabold tracking-tight uppercase text-[var(--text-muted)]">Academic Milestone</h3>
               <div className="h-px flex-1 bg-[var(--bg-muted)]" />
            </div>
                ...
                return (
                  <motion.div 
                    ...
                    className={`p-5 rounded-2xl border transition-all duration-300 relative group flex flex-col justify-between min-h-[120px] ${
                      isCompleted 
                        ? 'border-[var(--border-default)] bg-[var(--glass-bg)] backdrop-blur-xl hover:border-[var(--success)]/30 hover:shadow-sm' 
                        : isCurrent 
                        ? 'border-[var(--brand)]/30 bg-[var(--brand-bg)] ring-1 ring-[var(--brand)]/10 shadow-sm' 
                        : 'border-[var(--border-default)] bg-[var(--bg-base)] opacity-40'
                    }`}
                  >
                    ...
                    {isCompleted ? (
                      <div className="mt-2">
                        ...
                        <p className="text-[9px] font-mono text-[var(--text-muted)] mt-1 uppercase tracking-wider">{sem.sks} SKS Completed</p>
                      </div>
                    ) : isCurrent ? (
                      <div className="mt-2 flex flex-col gap-0.5">
                        <p className="text-sm font-bold text-[var(--brand)] flex items-center gap-1.5">
                          <span className="w-1.5 h-1.5 rounded-full bg-[var(--brand)] animate-pulse" />
                          ACTIVE
                        </p>
                        <p className="text-[9px] font-mono text-[var(--text-muted)] uppercase tracking-wider">In Progress</p>
                      </div>
                    ) : (
                      <div className="mt-2 flex flex-col gap-0.5">
                        <p className="text-sm font-bold text-[var(--text-muted)]">—</p>
                        <p className="text-[9px] font-mono text-[var(--text-muted)] uppercase tracking-wider font-semibold">Pending</p>
                      </div>
                    )}
                  </motion.div>
                );
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 rounded-[2rem] bg-[var(--bg-muted)] border border-[var(--border-default)] backdrop-blur-md">
            {[
              { label: 'Total Credits', value: academicStats.totalSKS, icon: Calculator, color: 'text-[var(--brand)]' },
              { label: 'Progression', value: `${academicStats.completedSemesters}/${semesterData.length}`, icon: BarChart2, color: 'text-[var(--brand)]' },
              { label: 'Latest IPS', value: academicStats.latestIPS.toFixed(2), icon: Star, color: 'text-[var(--success)]' },
              { label: 'Cumulative GPA', value: academicStats.ipk.toFixed(2), icon: Trophy, color: 'text-[var(--warning)]' }
            ].map((metric, i) => (
              <div key={metric.label} className="text-center p-2">
                <p className="text-2xl font-black text-[var(--text-primary)] tracking-tighter">{metric.value}</p>
                <p className="text-[9px] font-bold uppercase tracking-widest text-[var(--text-muted)] flex items-center justify-center gap-1">
          ...
          <motion.details initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.3 }} className="mt-4 rounded-2xl border border-[var(--border-default)] bg-[var(--bg-muted)] overflow-hidden">
            <summary className="px-5 py-3 text-xs font-medium text-[var(--text-muted)] cursor-pointer hover:text-[var(--text-primary)] transition-colors select-none flex items-center gap-2"><Calculator className="w-3.5 h-3.5" /> Bagaimana IPK dihitung?</summary>
            <div className="px-5 pb-4 text-xs text-[var(--text-muted)] space-y-2">
              <p><span className="font-semibold text-[var(--text-primary)]">IPK (Indeks Prestasi Kumulatif)</span> dihitung secara weighted berdasarkan SKS:</p>
              <div className="p-3 rounded-xl bg-[var(--bg-surface)] font-mono text-center border border-[var(--border-default)]">IPK = Σ(IPS<sub>i</sub> × SKS<sub>i</sub>) / Σ(SKS<sub>i</sub>)</div>
              <p className="leading-relaxed">Contoh saat ini: ({semesterData.filter(s => s.ips !== null).map(s => `${s.ips} × ${s.sks}`).join(' + ')}) / ({semesterData.filter(s => s.sks !== null && s.ips !== null).map(s => s.sks).join(' + ')}) = <span className="font-semibold text-[var(--success)]">{academicStats.ipk.toFixed(2)}</span></p>
              <p className="text-[11px] italic text-[var(--text-muted)]">* IPK akan otomatis ter-update setiap semester baru ditambahkan.</p>
            </div>
          </motion.details>
        </motion.section>
        ...
        <motion.section initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.5 }}>
          <h2 className="text-2xl font-bold mb-2 flex items-center gap-2"><Building2 className="w-6 h-6 text-[var(--brand)]" /> Organization</h2>
          <p className="text-[var(--text-muted)] text-sm mb-6">Campus organizations and community involvement.</p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {organizations.map((org, i) => (
              <motion.div key={org.name} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 + i * 0.1 }} whileHover={{ y: -4, scale: 1.01 }} className="group p-6 rounded-3xl border border-[var(--border-default)] bg-[var(--glass-bg)] backdrop-blur-xl transition-all duration-300 hover:shadow-[0_10px_40px_var(--brand-bg)] hover:border-[var(--brand)]/30 relative overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-r from-[var(--brand)]/0 via-[var(--brand)]/[0.03] to-[var(--brand)]/0 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000" />
                <div className="flex flex-col sm:flex-row items-start gap-5 relative z-10">
                  <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${org.color} flex items-center justify-center text-white flex-shrink-0 shadow-lg group-hover:scale-110 group-hover:rotate-6 transition-transform duration-300 ring-2 ring-[var(--bg-base)] inset-shadow-sm`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-[var(--text-muted)] mt-3 leading-relaxed font-medium">{org.desc}</p>
                  </div>
                </div>
              </motion.div>
            ))}
